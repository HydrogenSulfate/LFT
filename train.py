...
def train(local_rank, train_loader, device, net, criterion, optimizer, scaler=None, iters_to_accumulate=1):
    ...
    log_interval = 20
    for idx_iter, (data, label, coord) in tqdm(enumerate(train_loader), total=len(train_loader), ncols=70):
        data = data.to(local_rank)      # low resolution
        label = label.to(local_rank)    # high resolution
        coord = coord.to(local_rank)
