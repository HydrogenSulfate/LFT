...
def get_logger(log_dir, args):
    ...
    file_handler = logging.FileHandler('%s/%s.txt' %
                                       (log_dir, args.model_name))
...
def create_dir(args):
    ...
    task_path = 'SR_' + str(args.angRes) + 'x' + str(args.angRes) + '_' + str(
        args.scale_factor) + 'x'
...
def cal_metrics(args, label, out):
    if len(label.size()) == 2:  # only y case
        label = rearrange(label,
                          '(a1 h) (a2 w) -> 1 1 a1 h a2 w',
                          a1=args.angRes,
                          a2=args.angRes)
        out = rearrange(out,
                        '(a1 h) (a2 w) -> 1 1 a1 h a2 w',
    ...
    if len(label.size()) == 3:  # rgb case
        label = rearrange(label,
                          'c (a1 h) (a2 w) -> 1 c a1 h a2 w',
                          a1=args.angRes,
                          a2=args.angRes) # [1,3,A,H',A,W']
        out = rearrange(out,
                        'c (a1 h) (a2 w) -> 1 c a1 h a2 w',
                        a1=args.angRes,
                        a2=args.angRes) # [1,3,A,H',A,W']
    ...
    if len(label.size()) == 4:
        [B, C, H, W] = label.size()
        label = label.view((B, C, args.angRes, H // args.angRes, args.angRes,
                            H // args.angRes))
        out = out.view((B, C, args.angRes, H // args.angRes, args.angRes,
                        W // args.angRes))
...
def LFdivide(data, angRes, patch_size, stride):
    ...
    Args:
        data (torch.Tensor): [3, UH, VW]
        angRes (int): 5
        patch_size (int): 32
        stride (int): 16

    Returns:
        torch.Tensor: [numU, numV, 3, patch_size * angRes, patch_size * angRes]
    """
    ...
    dataE = torch.zeros(n_colors, hE * angRes, wE * angRes)
...
def ImageExtend(Im, bdr):
    ...
    Returns:
        torch.Tensor: [3, 3h, 3w]
    """
    n_colors, h, w = Im.shape
    ...
    Im_out = Im_Ext[:, h - bdr:2 * h + bdr, w - bdr:2 * w + bdr] # [3,3h,3w]
...
def LFintegrate(subLF, angRes, pz, stride, h0, w0):
    ...
    Returns:
        torch.Tensor: [angRes, angRes, n_colors, h0, w0]
    """
...
def rgb2ycbcr(x):
    ...
    y[:, :,
      0] = 65.481 * x[:, :, 0] + 128.553 * x[:, :, 1] + 24.966 * x[:, :,
                                                                   2] + 16.0
    y[:, :,
      1] = -37.797 * x[:, :, 0] - 74.203 * x[:, :, 1] + 112.000 * x[:, :,
                                                                    2] + 128.0
    y[:, :,
      2] = 112.000 * x[:, :, 0] - 93.786 * x[:, :, 1] - 18.214 * x[:, :,
                                                                   2] + 128.0
...
def ycbcr2rgb(x):
    ...
    y = np.zeros(x.shape, dtype='double')
    y[:, :, 0] = mat_inv[0, 0] * x[:, :, 0] + mat_inv[
        0, 1] * x[:, :, 1] + mat_inv[0, 2] * x[:, :, 2] - 16.0 / 255.0
    y[:, :, 1] = mat_inv[1, 0] * x[:, :, 0] + mat_inv[
        1, 1] * x[:, :, 1] + mat_inv[1, 2] * x[:, :, 2] - 128.0 / 255.0
    y[:, :, 2] = mat_inv[2, 0] * x[:, :, 0] + mat_inv[
        2, 1] * x[:, :, 1] + mat_inv[2, 2] * x[:, :, 2] - 128.0 / 255.0
...
def crop_center_view(data, angRes_in, angRes_out):
    ...
    data = data[:, :, (angRes_in - angRes_out) // 2 *
                patch_size:(angRes_in + angRes_out) // 2 * patch_size,
                (angRes_in - angRes_out) // 2 *
                patch_size:(angRes_in + angRes_out) // 2 * patch_size]
